...
def anno2df(
    anno_path, image_dir_path=None, cat_ids=None, start_image_id=0, start_anno_id=0
):
    """
    ...
    """
    coco_anno_dict, coco, image_dir_path = anno2coco(
        anno_path, image_dir_path=image_dir_path
    )
    ...
    for i, img_desc in enumerate(imglist):
        anno_ids = coco.getAnnIds(imgIds=[img_desc["id"]], catIds=cat_ids, iscrowd=None)
        ...
        dict_desc = {
            "new_image_id": int(i + start_image_id + 1),
        ...
        dict_desc = {**dict_desc, "img_desc": img_desc, "anno": anno}
...
def collec_newanno(path, dir_names, image_dir_path=None, cat_ids=None):
    ...
    for dir_name in dir_names:
        ...
        print(anno_path)
        df = correct_anno_img_names(
            anno_path, image_dir_path=image_dir_path, image_id=None
        )
        ...
        if df_.shape[0] > 0:
            ...
            print(
                f"images:{last_image_id-image_id_tmp}, instances:{last_anno_id-anno_id_tmp}"
            )
        else:
...
def _class_desc(annodf, data_desc):
    ...
    for class_id, class_name in zip(class_ids, class_names):
        data_desc["categories"] += [
            {"id": class_id, "name": class_name, "supercategory": ""}
        ]
...
def create_json(
    annodf, project_dir=None, new_anno_name="annotation.json", data_desc=None
):
    """
...
def merge_anno(path2anno, dir_names, image_dir_path, path2save):
    ...
    first_row["img_desc"]["file_name"] = Path(first_row["img_desc"]["file_name"]).name
    start_img_id = 1
    dict_desc = {
    ...
    for row in df.iterrows():
        ...
        if current_fname in list(df_new.file_name):
            ...
            old_dict["file_name"] = Path(old_dict["old_dict"]).name
            ...
            dict_desc = {
                ...
                "file_name": Path(old_dict["file_name"]).name,
            ...
            img_desc["file_name"] = Path(img_desc["file_name"]).name
            dict_desc = {
                "image_id": start_img_id,
