...
def list_dirs(path, exclude_if_startwith=(".", "__")):
    ...
    listdir = [
        ...
        if os.path.isdir(dname) and not dname.startswith(tuple(exclude_if_startwith))
    ]
    return listdir
...
def list_ext(dirpath, ext="txt"):
    ...
    list [strings]: list of file names
    """
    return [fname for fname in os.listdir(dirpath) if fname.lower().endswith(ext)]
...
def list_not_ext(dirpath, ext=IMAGE_AND_LABELS_EXTANTIONS):
    ...
    list [strings]: list of file names
    """
    return [fname for fname in os.listdir(dirpath) if not fname.lower().endswith(ext)]
